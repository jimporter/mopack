...
class TestFetch(CommandsTestCase):
    ...
    def test_removed_package(self):
        ...
        old_metadata = Metadata(self.pkgdir)
        ...
        with mock.patch('os.path.exists', return_value=False), \
             mock.patch('mopack.metadata.Metadata.try_load',
...
class TestResolve(CommandsTestCase):
    def test_empty(self):
        with mock.patch('mopack.log.info') as mlog, \
             mock.patch.object(Metadata, 'save') as msave:
    ...
    def test_package(self):
        ...
        metadata.add_package(DirectoryPackage(
            'foo', path='path', build='none', linkage='pkg_config',
